<template>
  <div class="sign-up">
    <h3>Grant administrator privileges</h3>
    <Message/>
    <input type="text" v-model="email" placeholder="Email"><br>
    <span class="c_button"><a class='button' :class="{blue: !loading, grey: loading, disabled: loading }" href="javascript:void(0);" @click="grant">Grant</a></span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions([
        'userGrantAdministrator',
        'updateMessage'
      ]),

      grant: function() {
        if (this.loading) return
        this.userGrantAdministrator(this.email)
      },
      goBack: function() {
        if (this.loading) return
